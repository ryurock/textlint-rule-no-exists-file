...
var report = function report(context) {
  ...
  return {
    [Syntax.Link](node) {
      ...
      if (node.url.match(/^\.\//g)) {
        var fileBasePath = path.dirname(filePath); // アンカー(#) 以降を除去する

        var resolvePath = path.join(fileBasePath, node.url.replace(/#(.*)/g, ''));
      ...
      if (node.url.match(/^\/(.*)/g)) {
        // アンカー(#) 以降を除去する
        var _resolvePath = path.join(currentPath, node.url.replace(/#.*/g, '')); // ファイルが存在した場合
